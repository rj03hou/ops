...
class Config:
    log_level = "ERROR"
    zabbix_url = "http://xxx/api_jsonrpc.php"
    zabbix_user = "sys"
    zabbix_password = "xxx"
    groupname = "db"
    ...
    #zabbix group ids:db=53,fe=24,ops=6,www=8,www_db=22,data=12,cos=67,mobile=17,giant=18
    zbgroups = {"fe":[24],"sa":[6],"web":[8,22],"data":["12"],"cos":[67],"mobile":[17],"giant":[18],"db":[53]}
    #linux_base=1,mysql_5002=144,mysql_slave=139
    zbtemplates=[{"templateid":144},{"templateid":139},{"templateid":1}]
...
class Zabbixtools:
    ...
    def host_update_groupid(self,hostid,groupids):
        data = json.dumps(
                {
                    "jsonrpc": "2.0",
                    "method": "host.update",
    ...
    def host_update_templateid(self,hostid,templateid):
        data = json.dumps(
                {
                    "jsonrpc": "2.0",
                    "method": "host.update",
...
#set the db host group & template
#return: json,if has key "error",there'is something wrong,else ok.
def set_db_group_and_template(hostname,group):
    zb = Zabbixtools()
    hostid=zb.host_get_id(hostname)
    if(-1 == hostid):
        return "{'error':{'message':'not found %s'}}"%hostname

    #set the group and move it into "db" group
    set_groups = list(Config.zbgroups[group])
    set_groups.extend(Config.zbgroups["db"])
    set_groups_format = []
    for groupid in set_groups:
        set_groups_format.append({"groupid":groupid})

    #{u'jsonrpc': u'2.0', u'id': 1, u'error': {u'message': u'Application error.', u'code': -32500, u'data': u'[ CHost::update ] You do not have enough rights for operation'}}
    res = zb.host_update_groupid(hostid,set_groups_format)
    if( res.has_key("error") ):
        return json.dumps(res)

    #set the template
    res = zb.host_update_templateid(hostid,Config.zbtemplates)
    if( res.has_key("error") ):
        return json.dumps(res)

    #if there is no error,just return the last operation result as this method result.
    return json.dumps(res)


def calc_all_db_group_disk_consume():
    zb = Zabbixtools()
...
def test():
    #print set_db_group_and_template("db612","web")
    zb = Zabbixtools()
    print zb.host_get("db612")
